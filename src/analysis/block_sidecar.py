...
class BlockSidecar:
    ...
    def generate(self, video_path: str, total_frames: Optional[int] = None) -> bool:
        ...
        key = hashlib.sha1(
            f"{os.path.abspath(video_path)}|{st.st_size}|{int(st.st_mtime)}|v21"
            .encode()
        ).hexdigest()[:16]
    ...
    def filter_intra_grid_for(self, frame_index: int) -> Optional[np.ndarray]:
        """AV1 filter-intra mode grid (0..4, -1 = not used) at MI granularity,
        or None (non-AV1 / not ready)."""
        fb = self._frame(frame_index)
        return fb.filter_intra if fb is not None else None

    def segment_id_grid_for(self, frame_index: int) -> Optional[np.ndarray]:
        """AV1 segment-id grid (0..7) at MI granularity, or None (non-AV1 / not
        ready)."""
        fb = self._frame(frame_index)
    ...
    def lr_for(self, frame_index: int):
        """AV1 frame-level loop-restoration (lr_type, lr_unit_size, lr_blob),
        or ((), (), b"")."""
        fb = self._frame(frame_index)
        if fb is None:
            return (), (), b""
        return fb.lr_type, fb.lr_unit_size, fb.lr_blob
